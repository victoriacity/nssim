...
vel = ti.Vector.field(DIM, dtype=ti.f32, shape=num_particles, needs_grad=True)
# color of particles
...
def init():
    pos.from_numpy(np.random.randn(num_particles, DIM).astype(np.float32) * 0.1 + 0.5)
    col.from_numpy(np.random.rand(num_particles, 3).astype(np.float32))
    vel.from_numpy(np.random.rand(num_particles, 3).astype(np.float32) * 0.02 - 0.01)
...
def integrate():
    for i in pos:
        for d in ti.static(range(DIM)):
            if (pos[i][d] < 0 and vel[i][d] < 0) \
                or (pos[i][d] > 1 and vel[i][d] > 0):
                vel[i][d] = -vel[i][d]
        pos[i] += vel[i] * dt
