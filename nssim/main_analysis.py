...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        if args.dim == 3:
            if args.method == "mpm":
                from simulator_3d import MPMSimulator3D
        ...
        while sim.frame < FRAME + FRAME_START and gui.running:
            ...
            if not paused:
                if 2 < sim.frame < args.warmup:
                    ...
                    sim.step()
                    if sim.frame >= FRAME_START:
                        color_traj.append(sim.col.to_numpy())
                        loss_traj.append(sim.loss[None])
                '''
        ...
        request_exit.wait()
        color_traj = np.stack(color_traj)
        loss_traj = np.array(loss_traj)
        np.save("color_traj-%dd-%s-lr%s.npy" % (args.dim, args.method, args.lr), color_traj)
        np.save("loss_traj-%dd-%s-lr%s.npy" % (args.dim, args.method, args.lr), loss_traj)
